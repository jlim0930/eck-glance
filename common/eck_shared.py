...
def get_gemini_review_prompt_template() -> str:
    """Return the configured Gemini review prompt template."""
    prompt_template = os.environ.get('ECK_GLANCE_GEMINI_REVIEW_PROMPT', '').strip()
    if not prompt_template:
        raise RuntimeError('Gemini review prompt is not configured. Set GEMINI_REVIEW_PROMPT in config.')
...
def build_gemini_review_prompt(summary: dict, user_notes: str = '') -> str:
    ...
    prompt_template = get_gemini_review_prompt_template()
    return (
        f"{prompt_template}\n\n"
