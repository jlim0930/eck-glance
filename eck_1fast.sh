...
WORKDIR="$(pwd)"
SCRIPTDIR=`echo ${0} | sed 's/eck_1fast.sh//g'`
export WORKDIR
export SCRIPTDIR
...
# agent.json
  ...
  if [ -e "${WORKDIR}/${namespace}/elasticmapsserver.json" ] && [ $(du "${WORKDIR}/${namespace}/elasticmapsserver.json" | cut -f1) -gt 7 ]; then
    ...
    emslist=`jq -r '.items[].metadata.name' "${WORKDIR}/${namespace}/elasticmapsserver.json"`
    for ems in ${emslist}
    do
      echo "    |---- [DEBUG] Parsing elasticmapsserver.json for ${entsearch}"
      ${SCRIPTDIR}/eck_ems_2.sh "${WORKDIR}/${namespace}/elasticmapsserver.json" "${ems}" > "${WORKDIR}/${namespace}/eck_elasticmapsserver-${ems}.txt"  &
    done
  fi
